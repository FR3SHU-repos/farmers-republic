...
export async function GET(
  ...
  try {
    ...
    // 🔧 Cast result explicitly so TS knows it's NOT an array
    ...
    const itemsForFarmer: MongoOrderItem[] = (order.items || []).filter(
      (it) => String(it.farmerId) === String(farmerId),
    );

    if (!itemsForFarmer.length) {
      return NextResponse.json(
        failure("No items for this farmer in this order"),
        { status: 404 },
      );
    }
    ...
    console.error("Farmer order detail error:", err);
    return NextResponse.json(
      failure("Failed to fetch order detail", err?.message || err),
      { status: 500 },
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    // 🔧 Again, cast so TS doesn't think it's an array
    ...
    console.error("Farmer order update error:", err);
    return NextResponse.json(
      failure("Failed to update order", err?.message || err),
      { status: 500 },
    );
  }
}
